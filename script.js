...
const squares = numbers.map(num => Math.pow(Number(num), 2));
console.log(squares);
...
const concatenatedString = alphabets.reduce((accumulator, currentValue) => {
...
const firstEvenNumber = number.find((number) => number % 2 === 0);
...
const hasFailingMarks = marks.some((mark) => mark < 45);
...
//  3
console.log(`Mr ${person.name} travels by ${person.vehicles.plane} called ${person.vehicles['plane']}-Air123`);
